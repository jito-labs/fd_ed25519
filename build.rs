...
fn main() {
    ...
    // Determine if AVX512 should be used
    ...
    if is_x86_64 {
        ...
        // On Linux, use -Wl,-z options
        if env::var("CARGO_CFG_TARGET_OS").unwrap_or_default() == "linux" {
            cc_build.flag("-Wl,-z,relro,-z,now");
        }
    } else if is_aarch64 {
        cc_build.flag("-DFD_HAS_ARM=1")
               .flag("-DFD_HAS_NEON=1");
    }
    ...
    // Platform-specific clang arguments for bindgen
    if is_x86_64 {
        bindgen_builder = bindgen_builder
        ...
        bindgen_builder = bindgen_builder
            .clang_arg("-DFD_HAS_ARM=1")
            .clang_arg("-DFD_HAS_NEON=1");
    }
